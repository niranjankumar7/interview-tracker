...
export function createJobReviewModal(props: JobReviewModalProps): HTMLElement {
  ...
  saveBtn?.addEventListener('click', () => {
    const company = (modal.querySelector('#company-input') as HTMLInputElement)?.value.trim();
    const role = (modal.querySelector('#role-input') as HTMLInputElement)?.value.trim();
    const location = (modal.querySelector('#location-input') as HTMLInputElement)?.value.trim() || undefined;
    
    if (!company || !role) {
      // Show validation error - fields are required
      const companyInput = modal.querySelector('#company-input') as HTMLInputElement;
      const roleInput = modal.querySelector('#role-input') as HTMLInputElement;
      
      if (!company) companyInput?.classList.add('error');
      if (!role) roleInput?.classList.add('error');
  ...
  editBtns.forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      const input = modal.querySelector(`#${field}-input`) as HTMLInputElement;
      if (input) {
        input.focus();
        input.select();
      }
