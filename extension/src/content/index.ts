 ...
 */
function isInterviewTrackerAuthPage(): boolean {
 ...
 */
function extractAuthData(): { token: string; user: { id: string; email: string; name: string | null } } | null {
  ...
  const globalAuth = (window as { __EXTENSION_AUTH__?: { token: string; user: { id: string; email: string; name: string | null } } }).__EXTENSION_AUTH__;
 ...
 */
async function sendAuthToBackground(data: { token: string; user: { id: string; email: string; name: string | null } }): Promise<void> {
  try {
 ...
 */
function monitorForAuthData(): void {
  // Check immediately
...
function initialize(): void {
  ...
  window.addEventListener('interviewTrackerAuth', ((event: CustomEvent) => {
    const data = event.detail as { token: string; user: { id: string; email: string; name: string | null } } | undefined;
    if (data?.token && data?.user) {
