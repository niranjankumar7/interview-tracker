...
export class GenericAdapter implements SiteAdapter {
   ...
   */
  private hasJobPostingSchema(document: Document): boolean {
    const scripts = document.querySelectorAll('script[type="application/ld+json"]');
    for (const script of scripts) {
      try {
        const data = JSON.parse(script.textContent || '');
        const schemas = Array.isArray(data) ? data : [data];
        if (schemas.some(s => s['@type'] === 'JobPosting' || (s['@graph'] && s['@graph'].some((g: { '@type': string }) => g['@type'] === 'JobPosting')))) {
          return true;
        }
      } catch {
        // Invalid JSON, skip
   ...
   */
  private hasJobPageIndicators(document: Document): boolean {
    const selectors = [
      '[data-testid*="job"]',
      '[class*="job-" i]',
      '[class*="position-" i]',
   ...
   */
  private extractFromJsonLd(document: Document): Partial<ExtractedJobData> & { confidence: number } | null {
    const scripts = document.querySelectorAll('script[type="application/ld+json"]');
        ...
        for (const schema of schemas) {
          ...
          for (const item of graph) {
            if (item['@type'] === 'JobPosting') {
              const result: Partial<ExtractedJobData> & { confidence: number } = {
                confidence: 0.9
              };
   ...
   */
  private extractFromOpenGraph(document: Document): { title?: string; siteName?: string; description?: string } | null {
   ...
   */
  private extractFromSelectors(document: Document): Partial<ExtractedJobData> & { confidence: number } | null {
   ...
   */
  private extractFromMetaSiteName(document: Document): string | null {
   ...
   */
  private extractTitleFromPage(document: Document): string {
    // Try h1 first
   ...
   */
  private calculateConfidence(data: { company: string; role: string; location?: string; jobDescription?: string; jsonLd?: boolean }): number {
    let score = 0.1; // Base score
    
    if (data.company && data.company !== 'Unknown Company') score += 0.2;
    if (data.role && data.role !== 'Unknown Position') score += 0.2;
    if (data.location) score += 0.15;
    if (data.jobDescription) score += 0.15;
    if (data.jsonLd) score += 0.2;
