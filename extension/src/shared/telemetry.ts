...
export interface TelemetryMetrics {
  ...
  adapters: Record<string, { attempts: number; successes: number; failures: number }>;
  daily_stats: Record<string, { attempts: number; successes: number }>;
  last_sync: string | null;
}

// User settings for telemetry
 ...
 */
export async function setTelemetrySettings(settings: Partial<TelemetrySettings>): Promise<void> {
 ...
 */
async function saveEvent(event: TelemetryEvent): Promise<void> {
 ...
 */
export async function getPendingEvents(): Promise<TelemetryEvent[]> {
 ...
 */
async function updateMetrics(event: TelemetryEvent): Promise<void> {
  const metrics: TelemetryMetrics = (await storage.get(TELEMETRY_METRICS_KEY)) || {
    total_events: 0,
    domains: {},
    adapters: {},
    daily_stats: {},
  ...
  if (event.event === 'extraction_attempt') adapterMetric.attempts++;
  if (event.event === 'extraction_success') adapterMetric.successes++;
  if (event.event === 'extraction_failure') adapterMetric.failures++;
  ...
  if (event.event === 'extraction_attempt') metrics.daily_stats[dateKey].attempts++;
  if (event.event === 'extraction_success') metrics.daily_stats[dateKey].successes++;
 ...
 */
async function maybeSyncEvents(): Promise<void> {
  const events = await getPendingEvents();
  const settings = await getTelemetrySettings();
 ...
 */
export async function syncEvents(): Promise<{ success: boolean; error?: string }> {
  const settings = await getTelemetrySettings();
  if (!settings.enabled) {
 ...
 */
export async function getDomainSuccessRate(domain: string): Promise<number> {
  const metrics = await getMetrics();
 ...
 */
export async function getDomainEditRate(domain: string): Promise<number> {
  const metrics = await getMetrics();
  const domainMetric = metrics.domains[domain];
 ...
 */
export async function getRecentFailures(limit: number = 10): Promise<TelemetryEvent[]> {
  const events = await getPendingEvents();
  return events
 ...
 */
export function startExtractionTracking(
  domain: string,
  adapter: string
): {
  ...
  return {
    async success(fields: string[], confidence: number) {
      if (completed) return;
      completed = true;
 ...
 */
export async function trackDuplicate(
  domain: string,
  adapter: string
): Promise<void> {
