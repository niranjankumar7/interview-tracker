...
// Re-export ExtractedJobData type for use in this module
 ...
 */
export async function generateFingerprint(data: JobData): Promise<string> {
 ...
 */
function generateComparableKey(data: JobData): string {
  const normalizedCompany = normalizeCompany(data.company);
  const normalizedRole = normalizeRole(data.role);
  const sourceUrl = normalizeUrl(data.jobDescriptionUrl);
 ...
 */
export function isDuplicate(newJob: JobData, existingJobs: JobData[]): boolean {
