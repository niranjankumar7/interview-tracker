 ...
 */
function authMessageListener(request, sender, sendResponse) {
    switch (request.type) {
        case 'AUTH_LOGIN_CALLBACK':
            handleLoginCallbackMessage(request.payload, sendResponse);
 ...
 */
async function handleLoginCallbackMessage(payload, sendResponse) {
    try {
 ...
 */
async function handleLogoutMessage(sendResponse) {
    try {
        await logout();
 ...
 */
function broadcastAuthState(type, user) {
